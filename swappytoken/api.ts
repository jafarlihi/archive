...
const yourInfuraKey = '56e568655e204db89bc397cd1a8da4e0'
...
const callData = tokens.map((token: Token) => ({
...
export async function fetchTokenPrices (): Promise<any> {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    prices[tokens[i].symbol] = price.toString()
    if (tokens[i].prices.length === 0 || !price.eq(tokens[i].prices.at(-1)!)) { tokens[i].addPrice(price) }
  }
  return prices
}
